  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        {/* Info factura */}
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 space-y-1">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono text-gray-900">{factura.numero_factura || `#${factura.id}`}</span>
          </div>
          <div className="flex justify-between text-sm">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-60 transition-colors"
            >
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        ...
        <div className="px-6 py-4 max-h-96 overflow-y-auto">
          {isLoading ? (
            <p className="text-sm text-gray-400 text-center py-8">Cargando pagos...</p>
          ) : lista.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-8">Sin pagos registrados</p>
          ) : (
            <div className="space-y-3">
              {lista.map((pago, i) => (
                <div key={pago.id || i} className="flex items-start justify-between p-3 bg-gray-50 rounded-xl">
                  <div className="space-y-0.5">
                    <p className="text-sm font-semibold text-gray-900">{COP(pago.monto)}</p>
                    <p className="text-xs text-gray-500 capitalize">
                      {(pago.metodo_pago || '').replace(/_/g, ' ')}
                    </p>
                    {pago.notas && (
                      <p className="text-xs text-gray-400 italic">"{pago.notas}"</p>
                    )}
                    {pago.registrado_por && (
                      <p className="text-xs text-gray-400">Por: {pago.registrado_por}</p>
                    )}
                  </div>
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Total cartera</p>
            <p className="text-xl font-bold text-gray-900 mt-0.5">{COP(totalCartera)}</p>
            <p className="text-xs text-gray-400 mt-0.5">{facturasFiltradas.length} facturas pendientes</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Cartera vencida</p>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5 flex items-start gap-4">
          ...
          <div>
            <p className="text-xs text-gray-400 font-medium uppercase tracking-wide">Clientes en mora</p>
            <p className="text-xl font-bold text-orange-600 mt-0.5">{clientesEnMora}</p>
            <p className="text-xs text-gray-400 mt-0.5">con saldo vencido</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 flex flex-wrap items-center gap-3">
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <span className="text-sm text-gray-600 font-medium">Solo vencidas</span>
        </label>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        {isLoading ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm text-gray-400 mt-3">Cargando cartera...</p>
          </div>
        ) : facturasFiltradas.length === 0 ? (
          <div className="py-16 text-center">
            <Clock className="w-10 h-10 text-gray-200 mx-auto mb-3" />
            <p className="text-sm text-gray-400">No hay facturas de cartera</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">N° Factura</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {facturasFiltradas.map(factura => {
                  ...
                  return (
                    <tr
                      key={factura.id}
                      className={`transition-colors hover:bg-gray-50/60 ${vencida ? 'bg-red-50/40' : ''}`}
                    >
                      <td className="px-4 py-3">
                        ...
                        {factura.cliente_documento && (
                          <p className="text-xs text-gray-400">{factura.cliente_documento}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            ...
                            className="px-2.5 py-1 border border-gray-200 text-gray-600 text-xs font-medium rounded-lg hover:bg-gray-50 transition-colors whitespace-nowrap"
                          >
                            Ver pagos
                          </button>
                        </div>
